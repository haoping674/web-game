...
Orchard Ten 與 Color Links 遊戲結束後自動檢查榜單，暫列前 10 名時可自選填寫 1–20 字的公開名字並登錄。首頁的「查看線上排行榜」可切換榜單；不需註冊或登入。
...
灰燼墓誌已移除線上排行榜，魂燼結算與本機進度照常保留。API 不再接受該榜單的查詢與登錄；既有資料庫可重新執行 `npm run db:migrate`，同步停用資料庫函式的登錄入口，歷史紀錄仍保留。
