...
type GameScreenProps = { game: GameState; dispatch: React.Dispatch<GameAction>; settings: GameSettings; tutorialOpen: boolean; onOpenSettings: () => void; networkNotice: NetworkNotice }
export function GameScreen({ game, dispatch, settings, tutorialOpen, onOpenSettings, networkNotice }: GameScreenProps) {
  ...
  return <section className="play-screen"><header className="play-topbar"><div className="brand-lockup"><span className="brand-mark" aria-hidden="true">✦</span><span>Orchard Ten</span></div><div><button type="button" className="text-button compact" onClick={onOpenSettings}>設定</button><button type="button" className="text-button compact" onClick={() => dispatch({ type: 'restart' })}>重新開始</button></div></header><section className="hud" aria-label="遊戲資訊"><div><span>分數</span><strong>{String(game.score).padStart(3, '0')}</strong></div><div className="hud-combo"><span>Combo</span><strong className={comboLevel}>{game.combo} <small>最高 {game.bestCombo}</small></strong><i className={game.combo > 0 ? '' : 'is-idle'} style={{ '--combo-progress': `${game.combo > 0 ? (comboRemaining / COMBO_WINDOW_MS) * 100 : 0}%` } as React.CSSProperties} aria-label={game.combo > 0 ? 'Combo 剩餘時間' : undefined} aria-hidden={game.combo === 0} /></div><div className="timer"><span>時間</span><Timer seconds={game.secondsLeft} urgent={game.secondsLeft <= 10} /></div><button type="button" className="icon-button" aria-label="暫停遊戲" onClick={() => dispatch({ type: 'pause', now: Date.now() })}>Ⅱ</button></section><div className="board-actions"><button type="button" className="quiet-button" disabled={game.hintsUsed >= HINT_LIMIT} onClick={useHint}>提示 {HINT_LIMIT - game.hintsUsed}/{HINT_LIMIT}</button>{!validMove && <button type="button" className="quiet-button" onClick={() => { dispatch({ type: 'reshuffle' }); setMessage('已重新排列未消除的水果。') }}>重新排列</button>}<span>{validMove ? '找到可行組合' : '暫時沒有可行組合'}</span></div><NetworkStatusToast notice={networkNotice} inline /><GameBoard board={game.board} onSelectionEnd={handleSelection} disabled={game.status !== 'playing' || tutorialOpen} hint={hint} animationsEnabled={settings.animationsEnabled} /><p className="selection-status" aria-live="polite">{message}</p>{settings.showSelectionHelp && <p className="shortcut-tip">拖曳可框選；鍵盤可用方向鍵移動，按 Enter 設定矩形兩端。</p>}</section>
}
