...
describe('result leaderboard', () => {
  ...
  it('reuses the id after reopening a saved Ashbound result', async () => {
    fetchMock.mockImplementation(() => Promise.resolve(reply(qualified)))
    const first = render(<LeaderboardPanel board="ashbound-souls" score={5} resultKey="ashbound-run-one" />)
    await screen.findByRole('textbox')
    const stored = localStorage.getItem('orchard-leaderboard-last-result')
    first.unmount()
    render(<LeaderboardPanel board="ashbound-souls" score={5} resultKey="ashbound-run-one" />)
    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2))
    expect(localStorage.getItem('orchard-leaderboard-last-result')).toBe(stored)
  })
  it('remembers a saved Ashbound ending even if another game changes the preferred nickname', async () => {
    ...
    const first = render(<LeaderboardPanel board="ashbound-souls" score={5} resultKey="completed-run" />)
    fireEvent.change(await screen.findByRole('textbox'), { target: { value: '原本名字' } })
    fireEvent.click(screen.getByRole('button', { name: '儲存名字與成績' }))
    await screen.findByText(/成績已登錄/)
    first.unmount()
    localStorage.setItem('orchard-leaderboard-name', '別局名字')
    render(<LeaderboardPanel board="ashbound-souls" score={5} resultKey="completed-run" />)
