...
export default function AshboundGame({ globalSettings, onProgressChange }: { globalSettings: GlobalSettings; onProgressChange: () => void }) {
  ...
  return <section className="ash-game" aria-label="灰燼墓誌遊戲">
    <header className="ash-heading">
      <div><p className="ash-overline">ASHBOUND / A ROGUELIKE CHRONICLE</p><h1>灰燼墓誌<span>亡者的下一頁，由你寫下。</span></h1><p className="ash-credit">啟發於 Buriedbornes</p></div>
    ...
    {!run ? <div className="ash-camp">
      ...
      <div className="ash-camp-copy">
        ...
        <p className="ash-muted">選擇職業，深入無盡墓穴。路線、戰利品與每一個回合，都可能改寫你的終局。</p>
        ...
        <p className="ash-record">最深 {state.best} 層 <span>／</span> 墓誌 {state.runs} 次 <span>／</span> 弒王 {state.wins} 次</p>
      </div>
    </div> : <>
