...
describe('moves, combo and hints', () => {
  ...
  it('increments combo in the mode window and clears it on expiration or invalid input', () => { const once = gameReducer(state(), { type: 'select', rect: successRect, now: 100 }); const twice = gameReducer({ ...once, board }, { type: 'select', rect: successRect, now: 100 + getComboWindowMs('classic', 1) - 1 }); expect(twice.combo).toBe(2); expect(gameReducer(twice, { type: 'tick', now: twice.comboDeadline! + 1 }).combo).toBe(0); expect(gameReducer(twice, { type: 'select', rect: { start: { row: 0, column: 0 }, end: { row: 0, column: 1 } }, now: 200 }).combo).toBe(0) })
