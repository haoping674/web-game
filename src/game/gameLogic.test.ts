...
describe('moves, combo and hints', () => {
  ...
  it('completes a full timed round and reaches the result state', () => { let current = state(); for (let second = 0; second < ROUND_SECONDS; second += 1) current = gameReducer(current, { type: 'tick', now: second * 1_000 }); expect(current).toMatchObject({ secondsLeft: 0, status: 'finished' }) })
